...
iter.SVD.core.fast.all.modified <- function(A,Kmax,tol=1e-5,max.iter=100,sparse=TRUE,init=NULL,verbose=FALSE,tau=0,p.sample=1){
  ...
  tmp.SVD <- list(u=svd.new$u[,1:Kmax],v=svd.new$v[,1:Kmax],d=svd.new$d[1:Kmax])
  result <- list(iter=NA,SVD=tmp.SVD,A=A.new,err.seq=NA,A.thr=A.new.thr)
  return(result)
}
...
ECV.block.modified <- function(A, max.K, cv = NULL, B = 3, holdout.p = 0.1, tau = 0, dc.est = 2,kappa = NULL){
  n <- nrow(A)
  ...
  #print(fast)
  result <- lapply(holdout.index.list, holdout.evaluation.fast.all.modified, A = A, max.K = max.K, tau = tau, dc.est = dc.est, p.sample = 1-holdout.p, kappa = kappa)
  dc.block.err.mat <- dc.loglike.mat <- bin.dev.mat <- roc.auc.mat <- impute.err.mat <- block.err.mat <- loglike.mat <- rep(0,B)
  no.edge.seq <- rep(0,B)
  ...
  output <- list(impute.err=mean(impute.err.mat),l2=mean(block.err.mat),dev=sum(loglike.mat),auc=mean(roc.auc.mat),dc.l2=mean(dc.block.err.mat),dc.dev=sum(dc.loglike.mat),sse=mean(impute.err.mat),auc.mat=roc.auc.mat,dev.mat=loglike.mat,l2.mat=block.err.mat,SSE.mat=impute.err.mat,dc.dev.mat=dc.loglike.mat,dc.l2.mat=dc.block.err.mat)
  
  if(output$dev>output$dc.dev){
    dev.model <- paste("DCSBM",max.K,sep="-")
  }else{
    dev.model <- paste("SBM",max.K,sep="-")
  }
  if(output$l2>output$dc.l2){
    l2.model <- paste("DCSBM",max.K,sep="-")
  }else{
    l2.model <- paste("SBM",max.K,sep="-")
  }
...
holdout.evaluation.fast.all.modified <- function(holdout.index, A, max.K, tau, dc.est, p.sample, kappa){
  n <- nrow(A)
  edge.index <- which(upper.tri(A))
  edge.n <- length(edge.index)
  A.new <- matrix(0,n,n)
  ...
  SVD.result <- iter.SVD.core.fast.all.modified(A.new,max.K,p.sample=p.sample)
  dc.block.sq.err <-  dc.loglike <- roc.auc <- bin.dev <- block.sq.err <- impute.sq.err <- loglike <- 0
  ...
  for(k in max.K){
    ...
    impute.sq.err <- sum((A.approx[Omega]-A[Omega])^2)
    ...
    roc.auc <- auc(aa)#as.numeric(tmp.roc$auc)
    #print(tmp.roc$auc)
    #print(auc(aa))
    ...
    bin.dev <- sum((response-trunc.predictors)^2)#-sum(response*log(trunc.predictors)) - sum((1-response)*log(1-trunc.predictors))
    if(k==1){
      pb <- (sum(A.new,na.rm=TRUE)+1)/(sum(!is.na(A.new)) -sum(!is.na(diag(A.new)))+1)
      if(pb < 1e-6) pb <- 1e-6
      if(pb > 1-1e-6) pb <- 1-1e-6
      A.Omega <- A[Omega]
      block.sq.err <- sum((pb-A[Omega])^2)
      loglike <- -sum(A.Omega*log(pb)) - sum((1-A.Omega)*log(1-pb))
    ...
    diag(P.hat) <- 0
    block.sq.err <- sum((P.hat[Omega]-A[Omega])^2)
    P.hat.Omega <- P.hat[Omega]
    A.Omega <- A[Omega]
    P.hat.Omega[P.hat.Omega < 1e-6] <- 1e-6
    P.hat.Omega[P.hat.Omega > (1-1e-6)] <- 1-1e-6
    loglike <- -sum(A.Omega*log(P.hat.Omega)) - sum((1-A.Omega)*log(1-P.hat.Omega))
    #print(proc.time() - ptm)
    ...
    if(k==1){
      ...
      dc.block.sq.err <- sum((pb-A[Omega])^2)
      P.hat.Omega <- P.hat[Omega]
      A.Omega <- A[Omega]
      P.hat.Omega[P.hat.Omega < 1e-6] <- 1e-6
      P.hat.Omega[P.hat.Omega > (1-1e-6)] <- 1-1e-6
      
      dc.loglike <- -sum(A.Omega*log(P.hat.Omega)) - sum((1-A.Omega)*log(1-P.hat.Omega))
      ...
      dc.block.sq.err <- sum((P.hat[Omega]-A[Omega])^2)
      P.hat.Omega <- P.hat[Omega]
      A.Omega <- A[Omega]
      P.hat.Omega[P.hat.Omega < 1e-6] <- 1e-6
      P.hat.Omega[P.hat.Omega > (1-1e-6)] <- 1-1e-6
